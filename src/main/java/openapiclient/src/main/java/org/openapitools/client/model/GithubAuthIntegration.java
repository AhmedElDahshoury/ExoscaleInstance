...
public class GithubAuthIntegration {
  ...
  * @throws IOException if the JSON Element is invalid with respect to GithubAuthIntegration
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GithubAuthIntegration.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in GithubAuthIntegration is not found in the empty JSON string", GithubAuthIntegration.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GithubAuthIntegration.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GithubAuthIntegration` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : GithubAuthIntegration.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!GithubAuthIntegration.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'GithubAuthIntegration' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<GithubAuthIntegration> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(GithubAuthIntegration.class));

       return (TypeAdapter<T>) new TypeAdapter<GithubAuthIntegration>() {
           @Override
           public void write(JsonWriter out, GithubAuthIntegration value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public GithubAuthIntegration read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of GithubAuthIntegration given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of GithubAuthIntegration
  * @throws IOException if the JSON string is invalid with respect to GithubAuthIntegration
  */
  public static GithubAuthIntegration fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GithubAuthIntegration.class);
  }

 /**
  * Convert an instance of GithubAuthIntegration to an JSON string
  *
  * @return JSON string
  */
